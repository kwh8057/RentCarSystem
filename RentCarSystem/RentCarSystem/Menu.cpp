...
int CAR_DISPLAY::Menu()//화면에 보여지는 것들은 이 클래스에서 만들어질것입니다.
{
	...
	printMenu(num, y);//num위치의 메뉴를 컬러로 보여주고, y값을 받아옵니다.
	...
	while (1)
	{
		...
		if (ch == 0xE0)//화살표키이면
		{
			...
			printMenu(num, yy); //선택이 바뀌었으니 컬러변경을 위해서 다시 출력
		}
		else if (ch == '1') return 1;
		else if (ch == ESC || ch == '2') return 0;
...
void CAR_DISPLAY::printMenu(int n, int &y)
{
	int i, x;
	const char *MainMenu[] = { "=========== R E N T   C A R ============",
		"|                                      |",
		"|            1. 렌      트             |",
		"|                                      |",
		"|            2. 종      료             |",
